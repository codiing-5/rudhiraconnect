...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const invites = await db_1.default.bloodBuddy.findMany({
...
router.post('/invite', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const inviter = await db_1.default.user.findUnique({ where: { id: inviterId } });
        ...
        const invitee = await db_1.default.user.findUnique({ where: { email } });
        if (invitee) {
            // Check if connection already exists
            const existingLink = await db_1.default.bloodBuddy.findFirst({
                where: {
                    OR: [
            ...
            const buddy = await db_1.default.bloodBuddy.create({
                data: {
                    inviterId,
...
router.post('/link-referral', async (req, res) => {
    try {
        ...
        // Verify both exist
        const inviter = await db_1.default.user.findUnique({ where: { id: inviterId } });
        const invitee = await db_1.default.user.findUnique({ where: { id: inviteeId } });
        if (!inviter || !invitee) {
