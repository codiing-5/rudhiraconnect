...
router.post('/check', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { age, weight, lastDonationDate, hemoglobin, feverLast14Days, pregnancyBreastfeeding, recentSurgery, recentTattooPiercing, currentMedication, chronicIllness, } = req.body;
        ...
        // 2. Weight (>= 45 kg)
        if (userWeight < 45) {
            if (result !== 'PERMANENT_DEFERRAL')
                result = 'TEMPORARY_DEFERRAL';
        ...
        const checkRecord = await db_1.default.eligibilityCheck.create({
            data: {
                userId,
                age: userAge,
                weight: userWeight,
...
router.get('/history', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const history = await db_1.default.eligibilityCheck.findMany({
            where: { userId },
...
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const lastCheck = await db_1.default.eligibilityCheck.findFirst({
            where: { userId },
            orderBy: { createdAt: 'desc' },
        });
