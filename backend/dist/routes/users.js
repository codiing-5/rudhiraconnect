...
router.get('/profile', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const eligibilityChecks = await db_1.default.eligibilityCheck.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
        });
        const lastCheck = eligibilityChecks[0] || null;
        // Fetch camp registrations
        const registrationsCount = await db_1.default.campRegistration.count({
            where: { userId },
        });
        // Fetch donations
        ...
        // Fetch certificates
        const certificatesCount = await db_1.default.certificate.count({
            where: { userId },
        });
        // Fetch Blood Buddy referrals
        const referrals = await db_1.default.bloodBuddy.findMany({
        ...
        const hasQuizBadgeNotification = await db_1.default.notification.findFirst({
            where: {
                userId,
